...
export const useCanvas = (GAME_WIDTH: number, GAME_HEIGHT: number) => {
  ...
  const drawRect = (x: number, y: number, width: number, height: number, color: string = "black") => {
    if (!checkCtx(ctx)) return;
    ctx.fillStyle = color;
    ctx.fillRect(x - width / 2, y - height / 2, width, height);
  };
  const drawImg = (x: number, y: number, width: number, height: number, img: HTMLImageElement) => {
    if (!checkCtx(ctx)) return;
    ctx.drawImage(img, x - width / 2, y - height / 2, width, height);
  };
  const drawCircle = (x: number, y: number, size: number, color: string = "black") => {
    if (!checkCtx(ctx)) return;
    ctx.beginPath();
  ...
  return { drawRect, drawImg, drawCircle, clear, setContext };
};
